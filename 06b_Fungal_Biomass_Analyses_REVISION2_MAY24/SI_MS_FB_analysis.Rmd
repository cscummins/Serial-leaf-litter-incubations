...
erg_fb_yr2_si_sub <- erg_fb_yr2_si %>% filter(!is.na(fungi_vial_mass.g))
erg_fb_yr2_si_sub <- erg_fb_yr2_si_sub %>% filter(!stream=="WS17")
#without the bag numbers that we did not collect samples for and without WS17, we are left with 297 samples, as expected (9 streams, 11 deployments, 3 samples per stream per deployment)
...
k_yr2_si <- read.csv("litter_breakdown_coarse_fine_yr2_all.3May24.csv")
...
merged_lb_erg.naomit <- na.omit(merged_lb_erg) #440 -> 248 - let's explore the data to ensure we understand WHY we have the sample size that we do here

merged_lb_erg <- merged_lb_erg %>% filter(!is.na(fungi_vial_mass.g)) #396 -> 297 as expected -- 297 was the total number of fungal biomass samples that we originally had, since we took samples from only 3 of 4 R fine mesh litterbags in YR2
merged_lb_erg <- merged_lb_erg %>% filter(!(vial_cap_litter.g=="missing/ not received"))#297 -> 281. This excludes all the fungal biomass samples that were missing/lost. Some of these were also "NA" for the fine-mesh breakdown data, meaning that perhaps the bag itself was lost or perhaps the sample was truly lost in transit
merged_lb_erg <- merged_lb_erg %>% filter(!(erg_per_sample_ug=="not extracted"))#281 -> 260. This excludes fungal biomass samples for which the sample arrived at the lab, but was not extracted, likely due to a QC flag like being left on the counter for too long before being frozen.
merged_lb_erg <- merged_lb_erg %>% filter(!is.na(ln_percent_afdm_rem_F))#260 -> 249. These are samples for which the fine-mesh breakdown data was missing, likely due to litterbag loss.
merged_lb_erg <- merged_lb_erg %>% filter(!is.na(afdm_g))#249 -> 248 as expected! One additional sample for which it looks like ergosterol extraction was also not performed. Likely that this sample was lost or compromised in the extraction process.
