...
*******Updated 3 May 24 with updated litter breakdown csvs -- nothing has changed in terms of the data, but we changed the calculations in year 1 and year 2 data carpentry files so that we were calculating absolute values of k based on multiplying by negative 1 instead of taking the absolute value. However, this doesn't change anything since all k-values were negative except one coarse-mesh k in year 1, which we excluded in the data carpentry steps for Year 1 (see "cleaning and calculations" Rmd)
...
#litter breakdown
lb_si_yr1 <- read.csv("litter_breakdown_coarse_fine_yr1_all.3May24.csv")
lb_si_yr1 <- lb_si_yr1 %>% mutate(date_coll=ymd(date_coll))
lb_si_yr1 <- lb_si_yr1 %>% mutate(date_dep=ymd(date_dep))


lb_si_yr2 <- read.csv("litter_breakdown_coarse_fine_yr2_all.3May24.csv")
...
write.csv(all_si_yr1_lb_phys_chem, "all_si_yr1_lb_phys_chem_3May24.csv")
write.csv(all_si_yr2_lb_phys_chem, "all_si_yr2_lb_phys_chem_3May24.csv")
```
...
write.csv(all_si_lb_phys_chem_yr1_yr2, "all_si_lb_phys_chem_yr1_yr2_3May24.csv")
