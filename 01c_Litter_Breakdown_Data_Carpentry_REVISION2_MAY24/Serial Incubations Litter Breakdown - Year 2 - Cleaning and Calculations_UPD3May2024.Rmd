...
#remove WS17
fine_mesh_data_yr2 <- fine_mesh_data_yr2 %>%
  filter(!stream %in% c("WS17"))
#880 -> 792

#remove bags with holes from the df
fine_mesh_data_yr2 <- fine_mesh_data_yr2[!(fine_mesh_data_yr2$hole=="hole"),]
#792 -> 780 -- 12 bags removed after final QC

#remove bags collected in the wrong months -- these have "QC" in the QC column
fine_mesh_data_yr2 <- fine_mesh_data_yr2[!(fine_mesh_data_yr2$QC=="QC"),]
#780 -> 773 as expected because one of the FM bags that was deployed in the wrong month had a hole.
...
#comparing these lists to the "holes" dataframes, we can see that the only ones removed from the final joined df are the 8 bags in both coarse and fine that were deployed in the wrong year and removed. There are no overlapping bags with holes in these dfs. 792-8=784, so the joined df has the correct number of observations!
...
sum(is.na(coarse_fine_data_yr2_all$percent_afdm_rem_C))
#91
sum(is.na(coarse_fine_data_yr2_all$percent_afdm_rem_F))
#75
...
sum(is.na(coarse_fine_data_yr2_all$ln_percent_afdm_rem_C))
cf_yr2_all_NaN_lnC <- coarse_fine_data_yr2_all %>% filter(ln_percent_afdm_rem_C %in% c("NaN", "-Inf", "Inf"))
#93 NA's. Also 1 -Inf (percent afdm remaining was 0) and 2 NaN (percent afdm remaining was negative)

coarse_fine_data_yr2_all$ln_percent_afdm_rem_F<-log(coarse_fine_data_yr2_all$percent_afdm_rem_F)
sum(is.na(coarse_fine_data_yr2_all$ln_percent_afdm_rem_F))
cf_yr2_all_NaN_lnF <- coarse_fine_data_yr2_all %>% filter(ln_percent_afdm_rem_F %in% c("NaN", "-Inf", "Inf"))
#79 NA's. Also 4 NaN (percent afdm remaining was negative)
...
coarse_fine_data_yr2_all <- coarse_fine_data_yr2_all %>%
...
Check for mass gains - there are none for year 2
```{r}
...
coarse_fine_data_yr2_all$abs_val_k_shred<-
  as.numeric(coarse_fine_data_yr2_all$abs_val_k_coarse-coarse_fine_data_yr2_all$abs_val_k_fine)
...
coarse_fine_data_yr2_all_negshred <- coarse_fine_data_yr2_all %>% filter(new_shredder_k<0)
#only 43 negative shredder k's in YR1
...
write.csv(coarse_fine_data_yr2_all, "litter_breakdown_coarse_fine_yr2_all.3May24.csv")
...
acer_coarse_mean_data_yr2 <- coarse_fine_data_yr2_all %>% filter(rhodo_acer=="A") %>% 
  filter(!abs_val_k_coarse %in% c("Inf", "NaN", "NA")) %>% 
  filter(!is.na(abs_val_k_coarse))

mean_a_k_coarse.2 <- mean(acer_coarse_mean_data_yr2$abs_val_k_coarse, na.rm=T)
sd_a_k_coarse.2 <- sd(acer_coarse_mean_data_yr2$abs_val_k_coarse, na.rm=T)

```

```{r}

acer_fine_mean_data_yr2 <- coarse_fine_data_yr2_all %>% filter(rhodo_acer=="A") %>% 
  filter(!abs_val_k_fine %in% c("Inf", "NaN", "NA")) %>% 
  filter(!is.na(abs_val_k_fine))

mean_a_k_fine.2 <- mean(acer_coarse_mean_data_yr2$abs_val_k_fine, na.rm=T)
sd_a_k_fine.2 <- sd(acer_coarse_mean_data_yr2$abs_val_k_fine, na.rm=T)

```

```{r}

acer_shred_mean_data_yr2 <- coarse_fine_data_yr2_all %>% filter(rhodo_acer=="A") %>% 
  filter(!abs_val_k_shred %in% c("Inf", "NaN", "NA")) %>% 
  filter(!is.na(abs_val_k_shred))

mean_a_k_shred.2 <- mean(acer_coarse_mean_data_yr2$abs_val_k_shred, na.rm=T)
sd_a_k_shred.2 <- sd(acer_coarse_mean_data_yr2$abs_val_k_shred, na.rm=T)
...
rhodo_coarse_mean_data_yr2 <- coarse_fine_data_yr2_all %>% filter(rhodo_acer=="R") %>% 
  filter(!abs_val_k_coarse %in% c("Inf", "NaN", "NA")) %>% 
  filter(!is.na(abs_val_k_coarse))

mean_r_k_coarse.2 <- mean(rhodo_coarse_mean_data_yr2$abs_val_k_coarse, na.rm=T)
sd_r_k_coarse.2 <- sd(rhodo_coarse_mean_data_yr2$abs_val_k_coarse, na.rm=T)

```

```{r}

rhodo_fine_mean_data_yr2 <- coarse_fine_data_yr2_all %>% filter(rhodo_acer=="R") %>% 
  filter(!abs_val_k_fine %in% c("Inf", "NaN", "NA")) %>% 
  filter(!is.na(abs_val_k_fine))

mean_r_k_fine.2 <- mean(rhodo_fine_mean_data_yr2$abs_val_k_fine, na.rm=T)
sd_r_k_fine.2 <- sd(rhodo_fine_mean_data_yr2$abs_val_k_fine, na.rm=T)

```

```{r}

rhodo_shred_mean_data_yr2 <- coarse_fine_data_yr2_all %>% filter(rhodo_acer=="R") %>% 
  filter(!abs_val_k_shred %in% c("Inf", "NaN", "NA")) %>% 
  filter(!is.na(abs_val_k_shred))

mean_r_k_shred.2 <- mean(rhodo_shred_mean_data_yr2$abs_val_k_shred, na.rm=T)
sd_r_k_shred.2 <- sd(rhodo_shred_mean_data_yr2$abs_val_k_shred, na.rm=T)
